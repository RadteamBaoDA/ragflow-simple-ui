...
describe('verify_mock_data script', () => {
  ...
  it('queries counts and exits 0 on success', async () => {
    ...
    vi.mock('../../src/db/index', () => ({ getAdapter: async () => { if (process.env.VERIFY_MOCK_BEHAVIOR === 'fail') throw new Error('db-fail'); return { queryOne: async () => ({ count: '1' }) } } }))
    // Mock the logger used by the script so we can assert the info call
    vi.mock('@/services/logger.service', () => ({ log: { info: vi.fn(), error: vi.fn() } }))
