...
const sessionConfig: session.SessionOptions = {
  store:
    config.sessionStore.type === "redis" && redisClient
...
const startServer = async (): Promise<http.Server | https.Server> => {
  ...
  server.listen(config.port, async () => {
    ...
    if (await checkConnection()) {
      ...
      // Clean up any jobs stuck in "converting"/"waiting" from previous runs
      const { converterQueueService } =
        await import("@/modules/converter/converter-queue.service.js");
      await converterQueueService.cleanupStuckJobs();

      // Start converter scheduler (polls every 60s for scheduled processing)
      converterWorkerService.startScheduler();
    } else {
  ...
  startServer()
    .then((server) => {
      const shutdown = async () => {
        ...
        converterWorkerService.stopScheduler();
        process.exit(0);
      };
