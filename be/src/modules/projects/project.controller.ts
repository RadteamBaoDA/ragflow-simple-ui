...
export class ProjectController {
  ...
  static async create(req: Request, res: Response) {
    try {
      ...
      const actor = { id: req.user.id, email: req.user.email, ip: req.ip };
  ...
  static async remove(req: Request, res: Response) {
    try {
      ...
      const actor = req.user
        ...
        : undefined;
      await projectService.remove(req.params.id, actor);
      res.status(204).send();
    } catch (error: any) {
  ...
  static async setPermission(req: Request, res: Response) {
    try {
      ...
      if (!grantee_type || !grantee_id) {
        res
          .status(400)
          .json({ error: "grantee_type and grantee_id are required" });
        return;
      }
      if (!req.user) {
        res.status(401).json({ error: "Unauthorized" });
  ...
  static async removePermission(req: Request, res: Response) {
    try {
      ...
      const actor = req.user
        ? { id: req.user.id, email: req.user.email, ip: req.ip }
        : undefined;
      await projectService.removePermission(req.params.permissionId, actor);
      res.status(204).send();
    } catch (error) {
