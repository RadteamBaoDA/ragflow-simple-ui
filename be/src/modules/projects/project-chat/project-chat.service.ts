...
export class ProjectChatService {
  ...
  async create(
    ...
    try {
      ...
      throw new Error(
        "Failed to create RAGFlow chat assistant. Please verify server connection.",
      );
    }

    // Create local record
    const chat = await ModelFactory.projectChat.create({
    ...
    if (actor) {
      await auditService.log({
        ...
        action: AuditAction.CREATE_CHAT,
  ...
  async update(
    ...
    if (existing.ragflow_chat_id) {
      try {
        ...
        if (data.ragflow_dataset_ids)
          ragflowUpdate.dataset_ids = data.ragflow_dataset_ids;
        ...
        if (Object.keys(ragflowUpdate).length > 0) {
          await ragflowProxyService.updateChatAssistant(
            serverId,
            existing.ragflow_chat_id,
            ragflowUpdate,
          );
        }
      } catch (err) {
    ...
    // Update local record
    const chat = (await ModelFactory.projectChat.update(id, {
      ...data,
      last_synced_at: new Date(),
      updated_by: actor?.id ?? userId,
    } as Partial<ProjectChat>)) as ProjectChat;
  ...
  async remove(
    ...
    if (serverId && existing.ragflow_chat_id) {
      try {
        await ragflowProxyService.deleteChatAssistants(serverId, [
          existing.ragflow_chat_id,
        ]);
      } catch (err) {
  ...
  async sync(id: string, serverId: string): Promise<ProjectChat> {
    ...
    if (!existing) throw new Error("Chat assistant not found");
    if (!existing.ragflow_chat_id)
      throw new Error("Chat has no RAGFlow assistant linked");
    ...
    if (!assistant)
      throw new Error("RAGFlow chat assistant not found on server");

    return ModelFactory.projectChat.update(id, {
      name: assistant.name || existing.name,
      ragflow_dataset_ids:
        assistant.dataset_ids || existing.ragflow_dataset_ids,
