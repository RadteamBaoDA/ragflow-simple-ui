# Generative RAG Orchestrator v2 — Research & Specification Document
### (For coding-agent plan mode — no implementation code; behavior contracts, decisions, and system prompts only)
...
1. **Discover the current codebase first.** Before writing any plan, locate and read:
   ...
   - The **agent standard** feature: where agent definitions/instructions are stored, how an agent is loaded, what fields an agent record has (instruction text, knowledge base ids, etc.).
...
2. **Map this spec onto what exists.** For each phase below, identify whether existing code can be refactored or must be newly created, and which existing modules are deleted/retired (notably the standalone outliner and the content-producing parts of the stitch/refine phase).
3. **Produce the detailed implementation plan** (file-by-file change list, new modules, retired modules, migration steps, test list) and only then implement.
4. **Treat every "MUST/MUST NOT" in this document as an acceptance criterion.** Section 10 consolidates them into a verification checklist.
...
**Agent Standard mode** — the agent has a predefined instruction (which may itself contain a skill description, `# User profiles`, `# Context`, `# Keyword`, and `# Output format`). The user prompts **freestyle natural language** OR optionally with:
...
Defects relative to the constraints: three sequential LLM gates before any content token is produced; retrieval is global rather than per-section; the final phase **adds content the user's `# Output format` never requested**; no sufficiency gate before generation; no agent-mode input handling.

### Target pipeline (4 logical phases)

```
PHASE 1  NORMALIZE INPUT
         deterministic header parsing + precedence merge
...
PHASE 3  PARALLEL PER-SECTION PIPELINE (bounded concurrency 3–5)
         per section: retrieve → rerank → sufficiency check
...
PHASE 4  VALIDATE-ONLY
         ...
         + deterministic citation-integrity check
         + ONE small LLM task-coverage call
...
**Phases removed:** standalone outliner (merged into Phase 2); global retrieve+rerank phases (folded into Phase 3 per section); the content-producing "stitch & refine / consistency pass" (replaced by Phase 4 validation-only).
...
Total LLM work is similar; the gain is **parallelism plus removed sequential gates**.
...
| `agentInstruction` | string \| absent | Raw agent text, agent mode only (used later as writer persona) |
...
Assembled markdown + the plan + per-section records + the **global citation registry** (citation number → chunk reference, for the UI's "show sources" panel — NOT injected into the document) + a `warnings` object: structure errors, citation errors, coverage verdict, regenerated section ids. Warnings never fail the request.
...
### Decision
The orchestrator **is reusable for Agent Standard mode with zero downstream changes**. All mode awareness lives in one normalizer that emits the canonical Request. Downstream phases MUST NOT branch on mode (except the writer, which receives the agent instruction as an opaque persona string).
...
2. **Precedence rules (normative):**

| Field | Chat mode | Agent mode |
|---|---|---|
| User profiles | required from user | from **agent instruction**; user cannot override |
...
| Output format | **required from user** | **user overrides agent** when both present and different (`outputFormatConflict=true`); agent's is the fallback |

3. **Chat-mode validation:** missing `# Task` or `# Output format` is a hard, user-visible error before any LLM call.
4. **Freestyle fallback (agent mode only):** if the user prompt contains none of the canonical headers, run exactly one small LLM extraction call (≤500 input tokens, schema-constrained JSON) producing `task`, `context`, `keyword[]`, `output_format`. Prompt: §9.1.
5. **Cost profile:** zero LLM calls when headers are present; the normalizer must complete in <100 ms in that path.

### Rejected alternatives
Separate orchestrators per mode (duplicated maintenance); injecting the agent instruction straight into the planner prompt (precedence becomes implicit and unauditable); always running the LLM extractor (1–2 s tax on every chat request for nothing).
...
1. **The standalone outliner phase is eliminated.** One LLM call produces classification + plan + outline metadata. On a 7B-class local model a second outlining call buys +0–5% structural quality for +5–15 s and an extra sequential gate — a bad trade under the 1–3 min budget.
2. **When `# Output format` is non-empty, the outline is NOT an LLM decision.** A deterministic parser derives section headings from the format text BEFORE the planner runs; the planner receives them as fixed input and only assigns per-section metadata. This eliminates the "invented section" failure class (LLM adding Introduction/Conclusion/References nobody asked for).
3. **Task taxonomy (7 classes)** — the planner classifies every request into exactly one:
...
This taxonomy is domain-neutral by construction (software, healthcare, legal, finance…). The planner prompt forbids domain assumptions; domain enters only through the task/context text and retrieval results.

4. **Three retrieval modes, assigned per section** (not per document — a single plan may mix all three):
...
| `none` | pure generation | no citations |

5. **Sub-queries live inside the plan.** 1–3 per retrieving section, each with a natural-language text (dense retrieval) and 0–4 keyword anchors (sparse/BM25). The user's `# Keyword` header is union-merged into the anchors downstream. NO separate HyDE / step-back / multi-query phases — each would add a full LLM call for marginal recall on this setup; revisit only if measured retrieval recall is inadequate.
6. **`dependsOn` is exceptional, not default.** Only when a section literally requires another section's text as input. The executor honors it; everything else runs in parallel.
...
### Rejected alternatives
Separate task-classifier call (doubles latency, negligible accuracy gain with constrained decoding); STORM-style perspective simulation (~10× cost, frontier-model technique); domain-specific planner prompts (anchoring; one structural prompt generalizes).
...
4. **A global chunk registry provides stable citation numbers.** Chunks are deduplicated by content hash across ALL sections; the first registration of a chunk assigns the next `[N]`; the same chunk cited from two sections yields the same number. The registry is concurrency-safe. The final registry (number → chunk) is returned with the result for the UI's source panel.
5. **Header leverage for retrieval quality:**
   - `# Keyword` → merged into every sub-query's sparse anchors.
   ...
   - Agent instruction → injected into the writer prompt as an opaque persona preamble (headers already consumed by the normalizer).
6. **Citations are enforced during generation, not post-hoc.** The writer's system prompt (§9.3) requires inline `[N]` after every factual claim for grounded sections. Post-hoc citation insertion is rejected: it needs a second LLM pass and measurably misattributes on small models.
7. **Deterministic citation validation after every write.** Extract all `[N]` tokens; keep only numbers present in the section's offered chunk set; **strip** invalid ones (never remap — remapping re-attributes a claim to an unrelated source); clean up orphaned spaces/punctuation. A grounded section with `mustCite=true` that ends with zero valid citations is flagged `no_citations` for Phase 4.
8. **Anti-lost-in-the-middle ordering.** When rendering numbered sources into the writer prompt, place the strongest chunks at the beginning AND end of the source block, weakest in the middle (e.g., rank order 1,4,5,3,2).
...
| Request slice (profile, task, context, format hint) | ~300 |
...
### Rejected alternatives
Single shared retrieval pool (generic context, worse grounding); post-hoc citation insertion (extra pass, misattribution); LLM-as-judge citation validation (regex + chunk-map is free and deterministic); per-section local citation numbering (same source gets different numbers across sections).
...
Rules:
- The sufficiency check is **set-level** (whole chunk set vs the section brief), chunks truncated to ~400 tokens each for the evaluator; ~1.5k in / ~80 out per call.
- `reference_inspired` and `none` sections SKIP the sufficiency check entirely.
...
### Decision
The old "Stitch, Validate, Refine" becomes **validate-only**. Assembly is plain concatenation of section bodies under their planned headings, in plan order.
...
5. The validator's only permitted actions: **flag**, and **trigger regeneration of specific failing sections** (maximum ONE repair round, then return with warnings).

### The three checks

| Check | Mechanism | Cost | Failure handling |
|---|---|---|---|
| (a) Structure conformance | Deterministic: when the outline was derived from `# Output format`, assembled headings must match the parsed headings exactly in count, order, and text (case-insensitive) | 0 | Report; regenerate offending sections |
...
| (c) Task coverage | ONE small LLM call (§9.6). Input is headings + first ~150 tokens of each section — NEVER the full document (~2–2.5k in / ~100 out, 3–6 s) | 1 small call | Returns `coversTask`, `missingAspects[]`, `sectionsToRegenerate[]` |
...
3. If output_format is empty, propose a minimal section structure
   (3-7 sections) appropriate to the task. Set
   outlineSource="proposed_by_llm".
4. For each section, choose retrievalMode:
   ...
   - none: no retrieval needed. Use only when the section is pure
     formatting, transitions, or creative writing with no factual claims.
5. For each section needing retrieval, write 1-3 subQueries. Each
...
6. Set targetTokens per section so the total fits the user's length
   intent (default 250-500 per section if unstated).
7. Set dependsOn only when a later section literally cannot be written
...
retrievalMode, subQueries, targetTokens, mustCite, minCitations, and
rationale for each. Set outlineSource to "derived_from_output_format".
```

**User message:** the five canonical fields, one per line, `(none)`/`(empty — propose 3-7 sections)` placeholders for blanks. Budget: ≤3k in / ≤1.5k out. Temperature ≤0.2.
...
7. Paraphrase; do not quote source text verbatim unless the brief says
   ...
   sources.
8. Neutral, factual tone. No marketing, no hedging filler.
...
**User message composition (order matters):** persona block (agent instruction slice in agent mode + "Reader profile: {userProfile}") → `USER TASK` → `SECTION TITLE` → `SECTION BRIEF` (plan rationale) → `TARGET LENGTH` → rolling summaries of prerequisite sections (labeled "do not repeat their content") → `SOURCES:` block with anti-lost-in-the-middle ordering. Temperature ~0.3.

### 9.3b Section writer — `reference_inspired` (rules 1–4 replaced; 5–8 and Output format shared)
...
### 9.3c Section writer — `none` (pure generation)
...
Do not write the section title or heading. Do not include citations.
Plain markdown prose, approximately {target_tokens} tokens; use lists
only if the brief explicitly asks for them. Stay strictly on the
user's TASK; do not pad, do not add disclaimers.
```

### 9.4 Sufficiency check (Phase 3, grounded sections; before generation)
...
**User message:** `SECTION TITLE`, `SECTION BRIEF`, `USER TASK`, then the chunk set each truncated to ~400 tokens, numbered. Budget ~1.5k in / ~120 out. Temperature 0. Empty chunk set short-circuits to insufficient without an LLM call.
...
address the user's TASK and the requested OUTPUT FORMAT. You see only
each section's heading and opening lines; judge coverage of topics,
not writing quality. Do not penalize sections marked
"Insufficient context in knowledge base for this section." - instead
list what they were supposed to cover under missingAspects.
...
**User message:** `TASK`, `OUTPUT FORMAT REQUESTED` (the plan's one-line summary), then per section: index, sectionId, heading, first ~150 tokens of body. Budget ≤2.5k in / ~200 out. Temperature 0.

---

## 10. Consolidated acceptance criteria (the coding agent's verification checklist)
...
- [ ] "Requirements → test cases"-style task → `transform_derive` + grounded_strict sections. "Old use cases → new use cases"-style task → `reference_inspired` sections. (Two integration fixtures.)

**Phase 3 — Section pipeline**
- [ ] Existing retriever and reranker are invoked directly (adapters at most); no retrieval logic reimplemented.
...
- [ ] Coverage auditor sees only headings + ~150-token openings.
...
- [ ] Structured logs at every phase boundary: phase name, duration, token in/out; wired into existing observability.
- [ ] Config surface: kb ids, concurrency (default 4), retrieve topK (20), rerank topK (5), NLI gate flag, coverage flag, max sections (10).

**Suggested implementation order:** types/contracts → header & format parsers (unit tests) → normalizer → constrained-JSON LLM wrapper → planner → chunk registry → section pipeline with sufficiency loop → citation validator → writers → validator + assembler + repair → routes/jobs/SSE → NLI gate → performance test. Retire the standalone outliner module and the content-producing stitch/refine code paths as part of the same change, with the old behavior kept behind a fallback flag only if the team requires a rollback path.
