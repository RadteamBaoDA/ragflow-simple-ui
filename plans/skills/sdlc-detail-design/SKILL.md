---
name: sdlc-detail-design
description: Generative RAG orchestrator artifact skill for Detail Design / Low-Level Design documents (LLD, detail design, class/function specs, API specification, database design, processing logic, pseudocode). Entry point for both the Phase-2 plan bias and the Phase-3 writer discipline. Structurally inert — never adds/renames/reorders sections, never overrides the resolved # Output format. Loads in chat generative mode; in agent mode it is a FALLBACK behind the agent instruction.
---

# Detail Design (Low-Level Design) — Artifact Skill

Artifact-specific guidance for generating a **Detail Design / LLD** in the generative
...
- **[write.md](write.md)** — Phase 3 discipline: class/function/API/DB specs consistent with the HLD and existing schemas, no-invented-fields/endpoints/tables, processing logic with error handling, citation strictness. Shapes the body (HOW), never the structure (WHAT).

## When this skill applies
The user (chat mode) or the resolved task asks for an implementable detail/low-level
design derived from a basic design and existing schemas.

## Mode rules (read first)
- **Chat generative mode** — no agent instruction exists; this skill is the primary
  source of LLD discipline. Both `plan.md` and `write.md` apply.
