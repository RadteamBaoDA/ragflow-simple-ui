---
name: sdlc-use-case
description: Generative RAG orchestrator artifact skill for Use Case Specifications (use case, UC, UML use case, actor/flow specification, scenario spec). Entry point for both the Phase-2 plan bias and the Phase-3 writer discipline. Structurally inert — never adds/renames/reorders sections, never overrides the resolved # Output format. Loads in chat generative mode; in agent mode it is a FALLBACK behind the agent instruction.
---

# Use Case Specification — Artifact Skill

Artifact-specific guidance for generating **use case specifications** in the
generative RAG orchestrator. This `SKILL.md` is the entry point; phase-specific
guidance lives in two referenced files, each loaded only by the phase that uses it:
...
- **[write.md](write.md)** — Phase 3 discipline: actor-system step pairs, alternative/exception flows, no-invented-actors/rules, citation strictness. Shapes the body (HOW), never the structure (WHAT).

## When this skill applies
The user (chat mode) or the resolved task asks for a use case specification — new
from exemplars, or derived from requirements/change requests.

## Mode rules (read first)
- **Chat generative mode** — no agent instruction exists; this skill is the primary
  source of use-case discipline. Both `plan.md` and `write.md` apply.
