...
description: Phase-3 writer resource for the sdlc-use-case skill (Use Case Specifications — UC, UML use case, actor/flow specification, scenario spec). Not independently triggered; appended to a section's writer brief only after SKILL.md selects this artifact. Supplies per-section writing discipline only. Structurally inert — shapes the section body (HOW), never the outline or headings (WHAT); never adds/renames/reorders sections; the resolved # Output format and agent instruction always win.
---

# Use Case Specification — Writer Resource (Phase 3)
...
(§4) before any writer runs — this resource only fills the body of a section that
...
sibling [plan.md](plan.md) (Phase 2).
