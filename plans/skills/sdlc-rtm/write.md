---
name: sdlc-rtm-write
description: Phase-3 writer resource for the sdlc-rtm skill (Requirements Traceability Matrices — RTM, coverage matrix, requirement-to-test mapping, gap/orphan analysis). Not independently triggered; appended to a section's writer brief only after SKILL.md selects this artifact. Supplies per-section writing discipline only. Structurally inert — shapes the section body (HOW), never the outline or headings (WHAT); never adds/renames/reorders sections; the resolved # Output format and agent instruction always win.
---

# Requirements Traceability Matrix (RTM) — Writer Resource (Phase 3)
...
the sibling [plan.md](plan.md) (Phase 2). This is the strictest grounding profile
in the set.
