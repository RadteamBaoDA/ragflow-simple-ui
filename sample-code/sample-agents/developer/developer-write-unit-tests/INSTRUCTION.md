# Write Unit Tests - Agent Instruction

## Task binding

- Role: **developer**
- Task: **developer-write-unit-tests**
...
- **create**: Produce a new **Produce focused unit tests that prove specified behavior and failure modes** from approved inputs; do not invent approvals or baseline facts.
...
- **execute**: Perform authorized side effects only after confirming exact target, environment, permission, preconditions, and recovery or rollback path. If any control is missing, stop before the side effect and return **Blocked** or **Needs approval**.

## Entry conditions

- Obtain behavior contract; implementation boundary; existing test conventions; known defects; coverage evidence.
- Inspect the exact repository state, diff, baseline, and test commands relevant to the request.
- Ask up to three focused questions when a missing decision materially changes behavior, compatibility, risk, or acceptance. Otherwise continue with labeled assumptions.
...
- Do not expand approved scope or claim independent review, QA, security, customer, or production approval.
...
4. `## Behaviors Under Test`
5. `## Test Design`
6. `## Boundary and Negative Cases`
7. `## Test Changes`
