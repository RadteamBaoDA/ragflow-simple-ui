...
```text
Participants: User, Frontend (FE), AgentHandler (AH), AIbitat (AB), Model, Tools

User -> FE: chat prompt
FE -> AH: grepAgents switches to agent mode, websocket opens
AH -> AB: init and createAIbitat with last 20 chats
AH -> AB: startAgentCluster sends USER message to agent
AB -> AB: fetchParsedFileContext injects parsed and pinned docs
AB -> Model: prompt plus attached_documents block
  [alt injected context is enough]
    Model -> AB: direct answer
  [alt more context needed]
    Model -> Tools: rag-memory search
      [alt chunks found] Tools -> Model: contextTexts plus citations
      [alt no chunks]    Tools -> Model: "no local context found, web search may be needed"
    [opt external data needed]
      Model -> Tools: web-browsing query
      Tools -> Model: normalized results plus citations
      Model -> Tools: web-scraping URL
      Tools -> Model: page content, summarized if too long
    Model -> AB: synthesized grounded answer
AB -> FE: textResponseChunk, citations, usageMetrics, chatId
FE -> User: renders answer and source panel
AB -> AB: chat-history persists text, sources, metrics
```
...
Supported provider selection is controlled by `agent_search_provider` and environment variables. The full engine switch in `web-browsing.js` handles 14 values: `serpapi`, `searchapi`, `serper-dot-dev`, `bing-search`, `baidu-search`, `serply-engine`, `searxng-engine`, `tavily-search`, `duckduckgo-engine`, `exa-search`, `perplexity-search`, `brave-search`, `crw-search`, `you-search`.
