...
bootMCPServers
  servers already running?
    |-- yes -> skip boot
    |-- no  -> read anythingllm_mcp_servers.json
      autoStart false?
        |-- yes -> record failed boot result (shows as stopped)
        |-- no  -> parse transport type (stdio, SSE, streamable HTTP)
          -> validate required fields
          -> start transport and connect MCP client (30s timeout)
            |-- failure -> record failure in mcpLoadingResults
            |-- success -> store client in mcps, record success
              -> activeMCPServers returns @@mcp_serverName
              -> agent setup: attachPlugins calls convertServerToolsToPlugins
                -> mcp.listTools
                  tool in suppressedTools?
                    |-- yes -> tool removed
                    |-- no  -> build plugin named serverName-toolName
                      -> use inputSchema as parameters, dereference $ref pointers
                      -> handler calls currentMcp.callTool
                      -> returnMCPResult stringifies safely
                      -> placeholder replaced with expanded tool names, flagged isMCPTool
```
...
5. Uses the MCP tool `inputSchema` (merged with a draft-07 `$schema`) as the function parameters. Providers dereference internal `$ref` pointers before sending schemas to the LLM (`aibitat/providers/helpers/dereferenceSchema.js`) since MCP schemas may use them but LLM tool APIs do not.
