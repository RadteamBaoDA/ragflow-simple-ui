...
```text
Frontend -> HTTP chat endpoint: streamChat prompt
HTTP chat endpoint -> HTTP chat endpoint: grepCommand then grepAgents
[alt not an agent request]
  HTTP chat endpoint -> Frontend: normal RAG chat continues
[alt agent request]
  HTTP chat endpoint -> DB: WorkspaceAgentInvocation.new with UUID
  [alt row creation fails]
    HTTP chat endpoint -> Frontend: statusResponse, fall back to default chat
  [alt row created]
    HTTP chat endpoint -> HTTP chat endpoint: cache attachments by UUID
    HTTP chat endpoint -> Frontend: SSE agentInitWebsocketConnection then close stream
    Frontend -> WebSocket endpoint: open /api/agent-invocation/uuid
    WebSocket endpoint -> AgentHandler: AgentHandler init
    AgentHandler -> DB: validate invocation not closed
    AgentHandler -> AgentHandler: resolve provider and model, get cached attachments
    WebSocket endpoint -> AgentHandler: createAIbitat with socket
    AgentHandler -> AIbitat: load agents and tools
    WebSocket endpoint -> AgentHandler: startAgentCluster
    Frontend -> WebSocket endpoint: follow-up prompts sent as awaitingFeedback
    [opt bail command such as stop or /reset]
      WebSocket endpoint -> AIbitat: abort
      WebSocket endpoint -> WebSocket endpoint: close socket
    WebSocket endpoint -> DB: mark invocation closed on disconnect
    WebSocket endpoint -> Frontend: socket close
    Frontend -> Frontend: AGENT_SESSION_END, append Agent session complete
```
