...
```text
Browser -> HTTP chat endpoint: POST stream-chat ("@agent ..." or automatic mode)
HTTP: grepAgents -> create workspace_agent_invocations row + cache attachments by uuid
HTTP -> Browser: SSE chunk { agentInitWebsocketConnection, websocketUUID }, then SSE closes
Browser -> WS /agent-invocation/:uuid: open websocket
WS -> AgentHandler: init() + createAIbitat()
AgentHandler -> AIbitat: register USER + @agent, attach resolved plugins (skills/flows/MCP/imported)
AIbitat: inject parsed files + pinned docs into latest user turn (<attached_documents>)
AIbitat -> Provider LLM: system prompt + history + tool definitions
[loop tool-call recursion, capped by AGENT_MAX_TOOL_CALLS (default 10)]
  LLM -> AIbitat: tool call
  AIbitat -> Tool plugin: handler(args)
    [opt sensitive tool]
      Tool -> Browser: toolApprovalRequest (120s timeout -> fail closed / denied)
      Browser -> Tool: toolApprovalResponse { requestId, approved }
  Tool -> AIbitat: result (+ citations, outputs, attachments)
  AIbitat -> LLM: tool result appended, next completion
[end loop]
LLM -> AIbitat: final text
AIbitat -> Browser: streamed chunks, cards, citations over websocket
AIbitat -> workspace_chats: chat-history plugin persists response + outputs
AIbitat -> Browser: interrupt -> awaiting feedback | TERMINATE -> session end
```
...
- Optional per-turn model router (`resolveRoute`) when workspace uses `anythingllm-router`. Because aibitat instantiates its own provider from the resolved route (the router object is not on the inference path), `use(modelRouterCooldown.plugin(onInferenceComplete))` re-stamps the router's sticky-route cooldown on every `onInterrupt`/`onTerminate` (`plugins/model-router-cooldown.js`; wired in `agents/index.js` and `ephemeral.js`).
- Registers `aibitat.toggleAgentTool` (→ `#toggleAgentTool`, `agents/index.js`): mid-session enable/disable of any skill identifier (plain, `parent#child`, `@@flow_`, `@@<hubId>`, MCP `<server>-<tool>` with `serverName`). Enable = resolve via `resolveAgentSkill` (`defaults.js`) + `#attachPluginByName` + push onto agent's function list; disable = `aibitat.removeFunction`. Driven by the frontend ToolsMenu via ws `toolToggle` frames (`plugins/websocket.js` `handleToolToggle`; relay priority in `endpoints/agentWebsocket.js`). Changes apply on the next turn.
...
Opt-in: `web-browsing` (search provider from settings; 14 engines: serpapi, searchapi, serper-dot-dev, bing-search, baidu-search, serply-engine, searxng-engine, tavily-search, duckduckgo-engine, exa-search, perplexity-search, brave-search, crw-search, you-search — `web-browsing.js`), `create-chart` (rechart), `sql-agent` (4 tools), `filesystem-agent` (10 tools), `create-files-agent` (5 generators: txt/pdf/docx/xlsx/pptx), `gmail-agent` (18), `outlook-agent` (10), `google-calendar-agent` (10), `request-user-input` (clarifying questions, gated by setting), `create-scheduled-job` (single-user mode only — `SINGLE_USER_ONLY_SKILLS` in `defaults.js`; creates cron-scheduled ephemeral agent runs with a pinned tool list discovered via `listTools: true`; local-time cron converted to UTC; registers job output via `registerOutput`).
Infrastructure plugins (not LLM tools): `websocket`/`http-socket` (transport), `chat-history` (persistence), `file-history` (dev/debug, not in plugin registry), `cli` (debug), `model-router-cooldown` (re-stamps router cooldown on `onInterrupt`/`onTerminate` — see §5). `router-classifier` exists in `plugins/` but is consumed by `utils/router/index.js`, not the agent registry.
...
Telemetry events (no OTEL; `server/models/telemetry.js`): `agent_chat_started` (`endpoints/agentWebsocket.js`), `agent_tool_call` (`aibitat/index.js`, both execution paths), `agent_generated_file_downloaded` (`endpoints/agentFileServer.js`).
