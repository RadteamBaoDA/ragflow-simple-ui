...
```text
[alt create-chart]
  Skill tool -> Socket: send rechartVisualize with type dataset title
  Socket -> Frontend: rechartVisualize event
  Frontend: render Chartable chat item
  Skill tool -> chat-history: set _replySpecialAttributes
[alt create-files]
  Skill tool -> storage generated-files: write fileType-uuid.extension
  Skill tool -> Socket: send fileDownloadCard with filename and size
  Socket -> Frontend: fileDownloadCard event
  Frontend: render FileDownloadCard
  Skill tool -> chat-history: registerOutput into _pendingOutputs
[alt _replySpecialAttributes present]
  chat-history: _storeSpecial with saveAsType and postSave
[alt no special attributes]
  chat-history: _store standard chat response
chat-history: include _pendingOutputs as response.outputs then _cleanup
Frontend -> agentFileServer: download via generated-files endpoint with storageFilename
agentFileServer: authenticate validate filename confirm chat ownership
agentFileServer -> Frontend: file
```
