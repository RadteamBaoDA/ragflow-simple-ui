...
```text
AIbitat -> AgentHandler: fetchParsedFileContext before each turn
AgentHandler -> ParsedFiles and DocumentManager: getContextFiles and pinnedDocs
ParsedFiles and DocumentManager -> AgentHandler: documents
AgentHandler -> AIbitat: append attached_documents block to user message
AgentHandler -> AIbitat: addDocumentCitations
AIbitat -> LLM: model turn
  [opt model decides extra retrieval needed]
    LLM -> rag-memory tool: call with action search or store
      [alt duplicated action and content]
        rag-memory tool -> LLM: duplicated call notice
      [alt search]
        rag-memory tool -> Vector DB: performSimilaritySearch
        Vector DB -> rag-memory tool: sources and context
        rag-memory tool -> AIbitat: add citations
        rag-memory tool -> LLM: combined context text
      [alt store]
        rag-memory tool -> Vector DB: write memory to workspace namespace
        rag-memory tool -> LLM: success or failure text
AIbitat -> chat-history: emit citations event and persist as response.sources
```
