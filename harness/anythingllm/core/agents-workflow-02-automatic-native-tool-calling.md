...
```text
Chat request
chatMode is automatic?
  |-- no  -> agent mode only via explicit agent command
  |-- yes -> Workspace.supportsNativeToolCalling
    workspace.agentProvider set?
      |-- yes -> provider chosen
      |-- no  -> workspace.chatProvider set?
        |-- yes -> provider chosen
        |-- no  -> use env LLM_PROVIDER -> provider chosen
    workspace.agentModel set?
      |-- yes -> model chosen
      |-- no  -> workspace.chatModel set?
        |-- yes -> model chosen
        |-- no  -> base model for provider -> model chosen
    provider is anythingllm-router?
      |-- yes -> router resolved from router_id or MODEL_ROUTER_ID?
        |-- no  -> returns false, normal RAG chat
        |-- yes -> check router fallback provider and model as proxy
                   -> instantiate via AIbitat.getProviderForConfig
      |-- no  -> instantiate via AIbitat.getProviderForConfig
    supportsNativeToolCalling true?
      |-- yes -> same websocket agent path as explicit agent command
      |-- no  -> normal chat, prompt-based untooled path still usable via explicit agent
```
