...
```text
Import from Community Hub
  manifest lists plugin.json?
    |-- no  -> import rejected
    |-- yes -> download zip into agent-skills dir
      all zip entries inside target folder?
        |-- no  -> Zip Slip rejected
        |-- yes -> extract files
          -> rewrite plugin.json (active false, correct hubId)
          -> delete zip in finally block
          -> admin toggles skill active via endpoint
          -> activeImportedPlugins scans folders
            manifest active true?
              |-- no  -> skill not listed
              |-- yes -> return @@hubId placeholder
                -> agent setup: attachPlugins resolves placeholder
                  handler.js exists?
                    |-- no  -> plugin skipped
                    |-- yes -> loadPluginByHubId validates path inside pluginsPath
                      -> delete require cache and require handler.js
                      -> register aibitat.function with manifest params and setup args
                      -> requestToolApproval appended last, cannot be overridden
```
