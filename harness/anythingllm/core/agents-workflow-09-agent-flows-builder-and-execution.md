...
```text
Admin builds flow in AgentBuilder
-> POST agent-flows save
-> Supported block types?
   |-- no  -> Reject save
   |-- yes -> Write storage/plugins/agent-flows/uuid.json
              -> Flow active flag not false?
                 |-- no  -> Not exposed as tool
                 |-- yes -> activeFlowPlugins returns @@flow_uuid
                            -> attachPlugins calls loadFlowPlugin
                            -> Sanitize name and map start variables to tool params
                            -> aibitat.use registers flow tool
                            -> Model calls flow tool with args
                            -> FlowExecutor initializes variables from defaults and args
                            -> [step loop] Expand step config with dollar-brace variable paths
                               -> Execute step: start, apiCall, llmInstruction, webScraping
                               -> Store result in resultVariable or responseVariable
                               -> Step has directOutput?
                                  |-- yes -> Set aibitat.skipHandleExecution true
                                             -> Return result directly, no further model pass
                                  |-- no  -> More steps?
                                             |-- yes -> back to [step loop]
                                             |-- no  -> Return full execution result to model
```
