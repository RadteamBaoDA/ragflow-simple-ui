...
```text
Tool wants to run risky action
requestToolApproval exists on runtime?
  |-- no  -> falls through as approved, e.g. scheduled jobs
  |-- yes -> AGENT_AUTO_APPROVED_SKILLS matches skill or contains all?
    |-- yes -> approved immediately
    |-- no  -> skill whitelisted for user or globally?
      |-- yes -> approved immediately
      |-- no  -> transport?
        |-- browser websocket -> send toolApprovalRequest with requestId, 120s timeout
        |     user response before timeout?
        |       |-- approve -> approved
        |       |     always allow checked?
        |       |       |-- yes -> POST agent-skills whitelist add, stored per user in
        |       |       |          multi-user or globally in single-user
        |       |       |-- no  -> one-time approval
        |       |-- reject  -> rejected
        |       |-- timeout -> rejected, fail closed
        |-- httpSocket with telegramChatId and worker IPC
        |     -> relay to TelegramBotService, inline keyboard, same requestId
        |        and 120s fail-closed timeout
        |-- httpSocket without interactive channel -> denied for safety
```
