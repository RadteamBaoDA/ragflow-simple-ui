...
```text
LLM -> request-user-input tool: call sub-tool with questions
request-user-input tool -> AIbitat: check _clarifyState per-turn cap
  [alt cap already hit]
    request-user-input tool -> LLM: proceed with best judgment
  [alt allowance remains]
    request-user-input tool -> WebSocket plugin: aibitat.requestUserClarification
    WebSocket plugin -> Frontend: clarificationRequest with requestId
    Frontend: create clarifyingQuestion chat item
    Frontend -> User: render ClarifyingQuestionCard survey
      [alt user submits or skips]
        User -> Frontend: answers or skip
        Frontend -> WebSocket plugin: clarificationResponse with requestId
      [alt timeout at 120s]
        WebSocket plugin: normalize answers as skipped
    WebSocket plugin -> request-user-input tool: resolved normalized answers
    request-user-input tool -> LLM: return answers
    AIbitat: buffer survey in _pendingClarifyingQuestionSurveys
    [note] chat-history persists clarifyingQuestions in response JSON
```
