...
```text
WORKSPACE_AGENT.getDefinition gathers sources:
  |-- agentSkillsFromSystemSettings
  |     |-- DEFAULT_SKILLS minus disabled_agent_skills -> combined identifier list
  |     |-- optional skills from default_agent_skills
  |           -> filter via SKILL_FILTER_CONFIG availability and disabled sub-skill lists
  |           -> combined identifier list
  |-- agent_clarifying_questions_enabled is true?
  |     |-- yes -> add request-user-input sub-tool and role prompt note -> combined identifier list
  |     |-- no  -> skip
  |-- ImportedPlugin.activeImportedPlugins gives @@hubId entries -> combined identifier list
  |-- AgentFlows.activeFlowPlugins gives @@flow_uuid entries -> combined identifier list
  |-- MCPCompatibilityLayer.activeMCPServers gives @@mcp_name entries -> combined identifier list

Combined identifier list -> later: attachPlugins in AgentHandler resolves each identifier:
  |-- parent#child   -> loads child plugin
  |-- @@flow_uuid    -> loads a flow plugin
  |-- @@mcp_name     -> expands to one tool per MCP tool
  |-- other @@hubId  -> loads imported skill
  |-- plain slug     -> loads built-in plugin
```
